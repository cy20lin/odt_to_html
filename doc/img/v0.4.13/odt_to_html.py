...
class ODTConverter:
    ...
    def _process_inline_content(self, element: ET.Element) -> str:
        ...
        for child in element:
            ...
            if (x or y) and anchor_type in ('paragraph', 'char'):
                # Shapes anchored TO a paragraph/char are positioned absolutely relative to the anchor
                element_style.append("position: absolute")
                if x: element_style.append(f"left: {x}")
                if y: element_style.append(f"top: {y}")
            elif anchor_type == 'as-char' or anchor_type is None:
                # As-char elements or unset (default) flow inline
