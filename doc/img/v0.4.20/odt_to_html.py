...
class ODTConverter:
    ...
    def _process_frame(self, frame: ET.Element) -> str:
        ...
        # Handle as-char anchors: inline element with baseline-relative offset
        is_as_char = (anchor_type == 'as-char')
        
        if is_as_char:
            style_parts.append("display: inline-block")
            style_parts.append("vertical-align: text-bottom")
            # Apply svg:y as baseline-relative offset via transform
            if y:
        ...
        if frame_content_parts:
            # Wrap in appropriate element: span for as-char (inline), div otherwise
